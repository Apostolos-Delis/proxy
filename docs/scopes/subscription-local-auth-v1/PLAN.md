...
- Add a provider credential source selector for subscription credentials:
  ...
  - **Import from Codex** reads Codex auth JSON from `PROXY_CODEX_AUTH_FILE`, `CODEX_HOME/auth.json`, or `~/.codex/auth.json`.
...
3. Proxy stores the encrypted ChatGPT token bundle and advances the API key wizard to provider binding.
4. If hosted device auth is unavailable, run `codex login` on the proxy host and choose **Import from Codex**.
5. Leave auth JSON at `~/.codex/auth.json`, or set `PROXY_CODEX_AUTH_FILE` to a specific auth JSON path.
...
3. Proxy stores the encrypted Claude Code OAuth token and advances the API key wizard to provider binding.
