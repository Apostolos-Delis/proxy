...
- `features.ts` / compression / attribution: branch on dialect + harness profile instead of surface literals (`tokenAttribution.ts`, `toolResultCompression.ts`, `promptArtifacts.ts`, `cacheBusts.ts`).
