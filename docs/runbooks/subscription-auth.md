...
the proxy on a personal/workspace subscription credential instead of the company API key. The feature
is internal-only and **off by default** behind `SUBSCRIPTION_OAUTH_ENABLED`.
...
**Company-key dependency:** flag-off traffic silently falls back to `ANTHROPIC_API_KEY` or
`OPENAI_API_KEY`. If those are unset or placeholders, disabling the flag turns subscription traffic
into 401s rather than a graceful fallback — keep valid company keys configured, or accept that flag-off
stops affected provider traffic.
...
3. Console → **Provider keys** → **Add provider key** → provider **openai** → auth type
   **Subscription** → paste the Codex access token and ChatGPT account ID. The server stores only the
