...
export const providerConnectionHealth = pgTable(
  ...
  (table): PgTableExtraConfigValue[] => [
    uniqueIndex("provider_connection_health_scope_idx").on(
      table.organizationId,
      table.workspaceId,
      table.providerConnectionId
    ),
    index("provider_connection_health_cooldown_idx").on(
      table.organizationId,
      table.workspaceId,
      table.cooldownUntil
    ),
    foreignKey({
      name: "provider_connection_health_workspace_fk",
      columns: [table.organizationId, table.workspaceId],
      foreignColumns: [workspaces.organizationId, workspaces.id]
    }).onDelete("cascade"),
    foreignKey({
      name: "provider_connection_health_connection_fk",
      columns: [table.organizationId, table.workspaceId, table.providerConnectionId],
      foreignColumns: [
...
export const deploymentHealth = pgTable(
  ...
  (table): PgTableExtraConfigValue[] => [
    uniqueIndex("deployment_health_scope_idx").on(
      table.organizationId,
      table.workspaceId,
      table.deploymentId
    ),
    index("deployment_health_lockout_idx").on(
      table.organizationId,
      table.workspaceId,
      table.lockoutUntil
    ),
    foreignKey({
      name: "deployment_health_workspace_fk",
      columns: [table.organizationId, table.workspaceId],
      foreignColumns: [workspaces.organizationId, workspaces.id]
    }).onDelete("cascade"),
    foreignKey({
      name: "deployment_health_deployment_fk",
      columns: [
        table.organizationId,
        table.workspaceId,
        table.deploymentId,
...
export const routeDecisions = pgTable(
  "route_decisions",
  {
    ...
    routerDecisionId: text("router_decision_id"),
    routerDecision: jsonb("router_decision").$type<Record<string, unknown>>().notNull().default({}),
...
export const events = pgTable(
  "events",
  {
    ...
    // organization-level identity events do not.
    workspaceId: text("workspace_id").references(() => workspaces.id, { onDelete: "set null" }),
