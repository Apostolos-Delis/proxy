...
CREATE TABLE console_agent_conversations (
  id text PRIMARY KEY,
  organization_id text NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  created_by_user_id text REFERENCES users(id) ON DELETE SET NULL,
  title text,
...
CREATE UNIQUE INDEX console_agent_conversations_org_id_idx
  ON console_agent_conversations (organization_id, id);
CREATE INDEX console_agent_conversations_org_created_idx
  ON console_agent_conversations (organization_id, created_at);
CREATE INDEX console_agent_conversations_org_creator_idx
  ON console_agent_conversations (organization_id, created_by_user_id);

CREATE TABLE console_agent_runs (
  id text PRIMARY KEY,
  organization_id text NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX console_agent_runs_org_id_idx
  ON console_agent_runs (organization_id, id);
CREATE UNIQUE INDEX console_agent_runs_active_idx
  ON console_agent_runs (organization_id, conversation_id)
  WHERE status = 'running';
CREATE INDEX console_agent_runs_conversation_idx
  ON console_agent_runs (organization_id, conversation_id);
CREATE INDEX console_agent_runs_org_status_idx
  ON console_agent_runs (organization_id, status);

CREATE TABLE console_agent_messages (
  id text PRIMARY KEY,
  organization_id text NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  conversation_id text NOT NULL,
  role text NOT NULL,
...
CREATE INDEX console_agent_messages_conversation_created_idx
  ON console_agent_messages (organization_id, conversation_id, created_at);
CREATE INDEX console_agent_messages_org_run_idx
  ON console_agent_messages (organization_id, run_id);

CREATE TABLE console_agent_run_events (
  id text PRIMARY KEY,
  organization_id text NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX console_agent_run_events_org_run_seq_idx
  ON console_agent_run_events (organization_id, run_id, seq);

CREATE TABLE console_agent_proposals (
  id text PRIMARY KEY,
  organization_id text NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX console_agent_proposals_dedupe_idx
  ON console_agent_proposals (organization_id, dedupe_key)
  WHERE status = 'pending';
CREATE INDEX console_agent_proposals_org_status_idx
  ON console_agent_proposals (organization_id, status);
CREATE INDEX console_agent_proposals_conversation_idx
  ON console_agent_proposals (organization_id, conversation_id);
CREATE INDEX console_agent_proposals_org_run_idx
