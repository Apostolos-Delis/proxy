...
**Subscription credentials (internal-only):** an engineer can paste an OpenAI Codex access token + ChatGPT account ID as a provider credential, then bind it to an API key they own. Claude `setup-token` credentials are additionally gated behind `SUBSCRIPTION_OAUTH_ENABLED=true`. Anthropic forwards as `Authorization: Bearer` instead of `x-api-key`; OpenAI HTTP Responses traffic forwards to `OPENAI_CHATGPT_BASE_URL` (default `https://chatgpt.com/backend-api/codex`) with `Authorization: Bearer` and `ChatGPT-Account-Id`. OAuth credentials are hard-rejected on keys the engineer doesn't own (no pooling). Unsetting the flag disables Claude subscription credentials only; OpenAI Codex subscription credentials remain enabled. This is internal-only and must never be exposed to external customers — read the [subscription auth runbook](docs/runbooks/subscription-auth.md) before enabling Claude subscription auth (risk rationale: [scope plan](docs/scopes/subscription-auth-v1/PLAN.md)).
