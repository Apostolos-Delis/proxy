...
const operations: Operation[] = [
  ...
  { name: "usage+timeseries (UsagePage shape)", query: `query { usage(groupBy: model, start: "${RANGE.start}", end: "${RANGE.end}") { data ${usageGroupSelection} totals ${usageGroupSelection} } usageTimeseries(groupBy: model, interval: day, start: "${RANGE.start}", end: "${RANGE.end}") { groups ${usageGroupSelection} points { ts totals ${usageGroupSelection} groups } } members { userId name email } apiKeys { id name revokedAt } }` },
  { name: "overviewPage shape", query: `query { overview { requestCount totals { totalTokens } cost { selected baseline savings } routeQuality { lowConfidenceCount cheaperLikelyWouldWorkCount cheapCausedRetriesOrRepairsCount } } requests { createdAt selectedCost baselineCost usage { totalTokens } } modelUsage: usage(groupBy: model) { data { key usage { totalTokens } cost { selected } } } }` },
  ...
  { name: "keysPage shape", query: "query { apiKeys { id name scopes userId routingConfigId routingConfig { id name status } createdAt expiresAt revokedAt lastUsedAt } routingConfigs { id name status assignedApiKeyCount activeVersion { id version } } }" }
];

const mutations: Operation[] = [
  { name: "mut updateSettings", query: "mutation { updateSettings(input: { schemaVersion: 1, classifier: { model: \"route-classifier-cheap\", timeoutMs: 1500, maxAttempts: 2, allowRedactedExcerpt: false }, budgets: { maxRoute: \"deep\" }, routeQuality: { lowConfidenceThreshold: 0.5 }, promptCapture: { promptCaptureMode: \"raw_text\", retentionDays: 30 } }) { organizationId } }" },
