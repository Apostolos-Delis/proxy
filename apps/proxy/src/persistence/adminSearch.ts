...
export type AdminSearchHit = {
  kind: "session" | "log" | "user" | "logical_model" | "api_key";
  id: string;
  title: string;
...
export async function searchAdminEntities(
  ...
  const patterns = { text, id: /[0-9_:-]/.test(query) ? text : null };
  const [sessions, logs, users, models, keys] = await Promise.all([
    ...
    searchLogicalModels(db, organizationId, workspaceId, patterns),
    searchApiKeys(db, organizationId, workspaceId, patterns)
  ]);
  return { query, results: [...sessions, ...logs, ...users, ...models, ...keys] };
}
...
async function searchSessions(db: ProxyDbSession, organizationId: string, workspaceId: string, patterns: SearchPatterns) {
  ...
  )).orderBy(desc(agentSessions.updatedAt)).limit(HITS_PER_KIND);
  return rows.map((session): AdminSearchHit => ({
    kind: "session",
    id: session.id,
    title: session.externalSessionId ?? session.id,
    subtitle: session.surface,
...
async function searchLogs(db: ProxyDbSession, organizationId: string, workspaceId: string, patterns: SearchPatterns, query: string) {
  const rows = await db.select({ artifact: promptArtifacts, request: requests, decision: routeDecisions })
    ...
    )).orderBy(desc(promptArtifacts.createdAt)).limit(HITS_PER_KIND);
  return rows.map((row): AdminSearchHit => {
    ...
    return {
      ...
      subtitle: [
        row.decision?.requestedLogicalModel ?? row.request.requestedLogicalModel,
...
async function searchUsers(db: ProxyDbSession, organizationId: string, workspaceId: string, patterns: SearchPatterns) {
  const requestRows = db.select({ one: sql`1` }).from(requests).where(and(
    workspaceScope(requests, organizationId, workspaceId),
    eq(requests.userId, usersTable.id)
  ));
  const rows = await db.select({ user: usersTable, member: organizationMembers })
    .from(usersTable)
    ...
    )).orderBy(desc(usersTable.updatedAt)).limit(HITS_PER_KIND);
  return rows.map((row): AdminSearchHit => {
    ...
    return {
      ...
      id: row.user.id,
      title,
      subtitle: [row.user.email !== title ? row.user.email : null, row.member?.role].filter(Boolean).join(" | ") || null,
...
async function searchLogicalModels(db: ProxyDbSession, organizationId: string, workspaceId: string, patterns: SearchPatterns) {
  const rows = await db.select().from(logicalModels).where(and(
    workspaceScope(logicalModels, organizationId, workspaceId),
    anyOf([
      patterns.id ? ilike(logicalModels.id, patterns.id) : null,
      ilike(logicalModels.name, patterns.text),
      ilike(logicalModels.slug, patterns.text),
      ilike(logicalModels.description, patterns.text)
    ])
  )).orderBy(desc(logicalModels.updatedAt)).limit(HITS_PER_KIND);
  return rows.map((model): AdminSearchHit => ({
    kind: "logical_model",
    id: model.id,
    title: model.name,
    subtitle: model.description ?? model.slug,
    status: model.status,
    snippet: null,
...
async function searchApiKeys(db: ProxyDbSession, organizationId: string, workspaceId: string, patterns: SearchPatterns) {
  const rows = await db.select({ apiKey: apiKeys, accessProfileName: accessProfiles.name })
    .from(apiKeys)
    ...
    )).orderBy(desc(apiKeys.createdAt)).limit(HITS_PER_KIND);
  return rows.map((row): AdminSearchHit => ({
    kind: "api_key",
    id: row.apiKey.id,
    title: row.apiKey.name,
    subtitle: row.accessProfileName,
